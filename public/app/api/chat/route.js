...
const evaluateResponse = (conversation, extractedData) => {
  const metrics = {
...
const advancedReasoningPrompt = (messages, extractedData, metrics) => {
...
const extractCustomerInfoFromConversation = (conversationText) => {
  const data = {}
...
const classifyServiceType = (text) => {
...
const extractCustomerInfo = (messages) => {
  const conversationText = messages.map((m) => m.content || m.text).join(" ")
...
const isAskingAboutPOOrderStatus = (messages) => {
...
const isAskingForRepresentative = (messages) => {
  const conversationText = messages
    .map((m) => m.content || m.text)
    .join(" ")
    .toLowerCase()
...
const shouldDirectToCTA = (messages, extractedData) => {
  const conversationText = messages
    .map((m) => m.text)
    .join(" ")
    .toLowerCase()
        ...
        try {
          await fetch(`${req.url.replace("/api/chat", "/api/leads")}`, {
            ...
            body: JSON.stringify({
              ...
              notes: `Representative request from chat\n\n${messages.map((m) => `${m.sender || "user"}: ${m.text}`).join("\n")}`,
            }),
          })
        } catch (error) {
